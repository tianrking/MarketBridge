...
Version `v0.0.6` is shipped as downloadable binary packages from GitHub Actions
and GitHub Releases.
...
| Platform | File |
|---|---|
| Linux 64-bit x86 | `market-bridge-v0.0.6-linux-x86_64.tar.gz` |
| Linux 32-bit x86 | `market-bridge-v0.0.6-linux-i686.tar.gz` |
| macOS Intel | `market-bridge-v0.0.6-macos-x86_64.tar.gz` |
| macOS Apple Silicon | `market-bridge-v0.0.6-macos-aarch64.tar.gz` |
| Windows 64-bit | `market-bridge-v0.0.6-windows-x86_64.zip` |
...
| History candles | `/v1/history/candles` | Binance/OKX/Bybit/Coinbase public history | raw normalized | On-demand `spot`, `futures/perp`, `mark`, `index`, `premiumIndex` where available, and `funding_rate` candles; Binance/Bybit/OKX funding history accepts bounded `pages` (1–48), Binance OHLC candle requests accept bounded sequential pages, while Coinbase is spot-only and capped at 300 rows. `coverage_detail` reports requested/covered range, requested pages and possible truncation. |
| Historical taker volume | `/v1/history/taker-volume` | Binance public futures data | raw normalized | Aggregate taker buy/sell volume and imbalance; Binance accepts bounded `pages` (1–96) for sequential windows. The USDⓈ-M path derives the fields from official futures kline taker-buy columns and labels `unit=base_asset`; it is not trader identity or intent. |
| Historical liquidations | `/v1/history/liquidations` | OKX/CoinEx public liquidation history | raw normalized | Bounded liquidation details with normalized side, position side, price, quantity and timestamp; CoinEx accepts bounded `pages` (1–48), while OKX remains a recent provider window; `coverage_detail` reports the requested/covered range and possible truncation. |
...
| Historical account ratio | `/v1/history/account-ratio` | Binance/Bybit public positioning context | raw normalized | Provider-specific account/holder ratios with normalized imbalance, source semantics and bounded coverage; Binance accepts `scope=global / top_trader / top_trader_position` and bounded `pages` (1–48); not notional positioning or trader intent. |
...
| Historical basis | `/v1/history/basis` | Binance public futures data | raw normalized | Provider basis, index/futures prices, basis rate and annualized rate with bounded coverage; accepts `pages` (1–48) for sequential windows; not simultaneous bid/ask execution. |
...
- `market=spot|perp`
- `symbol`
- `interval=1m|3m|5m|15m|30m|1h|4h|1d|1w` for providers that expose the requested cadence; Binance weekly candles are normalized to a seven-day interval.
- `start_ms`, `end_ms`
...
| Data | Endpoint | Notes |
|---|---|---|
| Options chains | `/v1/options/chains` | Deribit/OKX/Bybit/Binance REST cache; rows are v1 envelopes with mark IV, strike, expiry, underlying and optional greeks/open interest. Deribit gamma research can use bounded `/options/deribit/book` enrichment because the summary endpoint may not carry greeks. |
...
| GET | `/v1/history/liquidations` | Bounded public OKX/CoinEx liquidation history (`pages` 1–48 for CoinEx; OKX recent-window only) for research replay. |
...
| GET | `/v1/research/squeeze/scan` | Read-only squeeze scan over symbols observed by the current process. |
| POST | `/v1/research/squeeze/archive` | Append-only local archive of the current squeeze scan evidence. |
| GET | `/v1/reference/supply` | Explicit identity-bound circulating supply and market-cap references. |
| GET/POST | `/v1/reference/venue-asset-status` | Query or ingest auditable venue deposit/withdrawal status evidence. |
