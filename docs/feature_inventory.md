...
| Historical liquidations | `/v1/history/liquidations` | implemented | keyless | Bounded OKX/CoinEx public liquidation history; CoinEx supports bounded `pages` (1–48), while OKX remains a recent-window feed; not a complete cross-venue ledger. |
...
| Historical account ratio | `/v1/history/account-ratio` | implemented | keyless | Binance global, top-trader account or top-trader position share (`scope`) with bounded `pages` plus Bybit holder-count buy/sell ratio; provider semantics, cursor and coverage metadata remain explicit; not a complete notional-position ledger. |
...
| Historical basis | `/v1/history/basis` | implemented | keyless | Binance public futures/index basis with basis rate, annualized rate and bounded `pages` (1–48) coverage metadata; not simultaneous executable legs or carry PnL. |
| Historical funding | `/v1/history/candles?candle_type=funding_rate` | implemented | keyless | Binance/Bybit/OKX and Hyperliquid public funding history; Binance/Bybit/OKX expose bounded pages, while provider timestamps and inferred adjacent schedules remain explicit. |
...
| DeFi yield context | `/v1/external/defi-yields` | implemented | keyless | DefiLlama pool APY, TVL, base/reward components and stablecoin flags; provider snapshot, not guaranteed yield or protocol safety. |
...
| Meteora | implemented | implemented | partial | keyless | Official DLMM pool snapshots plus normalized quote, TVL, 24h volume/fees, fee/TVL, dynamic-fee, bin-step and pagination context. |
| Orca | implemented | implemented | partial | keyless | Official Whirlpool pool snapshots plus TVL/volume/fee and CLMM risk context. |
...
| DeFi native state | Pool liquidity, route depth, swaps/trades | partial | DexScreener-backed pool sources emit liquidity, volume, and swap-count metrics; Uniswap V3 subgraph emits liquidity/TVL/volume/txCount; Jupiter emits a configured read-only quote-size/route-impact ladder; Raydium API v3 emits paginated pool aggregates and top-pool concentration; Orca and Meteora public APIs emit native CLMM/DLMM pool context. Protocol-native swap streams remain connector-specific extensions. |
...
| Cross-platform release binaries | implemented | P1 | GitHub Actions builds v0.0.6 Linux/macOS/Windows packages with binary, configs, README, and docs. |
