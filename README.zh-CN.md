...
| 历史基差 | Binance futures/index basis、basisRate、年化基差率，支持 `pages` 1–48 | `GET /v1/history/basis` | 暂无直接流 | 按需请求，保留 provider coverage；不是同步可成交套利 | 否 |
...
| GET | `/v1/history/candles` | 按需查询 Binance/OKX/Bybit/Coinbase 公共 candles；Coinbase 仅 spot、单页最多 300 行；返回 `coverage_detail`，funding history 附逐点 schedule。 |
| GET | `/v1/history/liquidations` | OKX/CoinEx 有界公开清算历史（CoinEx 支持 `pages` 1–48，OKX 仅 recent window），供回放使用；其他 venue 缺口保持显式。 |
...
同系列还提供 generic imbalance/funding response recorder/replay：分开保留 bid/ask 压力、资金费率冲突和普通盘口状态，
再测量之后固定窗口的 BTC 响应。
...
现在还提供资金费率价差响应回放：按两个交易所各自逐点结算间隔年化资金费率，比较极端或突然扩大的
价差与普通窗口之后的 BTC 绝对波动。这是 carry 上下文研究，不是方向预测、资金费收入计算或可执行对冲。
...
同系列还提供 price-cluster response recorder/replay：把已发生清算成交的价格带集中度与滚动 burst 名义金额分开检验，
不重建潜在热图清算墙。
...
Meteora 现在也接入官方 DLMM `/pools` API：将分页、TVL、24 小时 volume/fees、fee/TVL、dynamic fee、bin step 和 blacklist
上下文归一化，配套 `examples/crypto/defi/crypto_meteora_dlmm_*` 的 Python monitor/recorder/replay 与 BTC 响应研究。
分页未完整时始终 observe-only；它是数据与研究案例，不构造 swap、不签名钱包、不下单。
...
升级成稳定状态。
现在还提供 stress-response recorder/replay：把每个风险上下文快照与永续报价配对，比较压力、观察和普通状态之后的
BTC 有符号/绝对波动；不会把成交成本转成方向信号或路由信号。
微结构系列还新增 liquidity-sandwich response pair：检验买卖两侧近盘口深度都明显且点差较窄时，后续 BTC 绝对响应
是否不同于普通盘口。显示墙体只是一瞬间的快照证据，不是持续流动性、价格方向或区间交易指令。
