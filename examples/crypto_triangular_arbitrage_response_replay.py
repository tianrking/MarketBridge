#!/usr/bin/env python3
"""Replay BTC response after frozen triangular quote-edge states."""
...
def edge_state(observation, min_net_edge_bps):
    ...
    if edge is None:
        return "missing_triangle_edge"
    return "qualifying_triangle_edge" if edge >= min_net_edge_bps else "edge_below_paper_hurdle"
...
def summarize_records(records, horizon_records, min_net_edge_bps, min_observations):
    ...
    states = ("qualifying_triangle_edge", "edge_below_paper_hurdle", "missing_triangle_edge")
    ...
    return {
        ...
        "qualifying_aligned_windows": by_state["qualifying_triangle_edge"]["observations"],
        "verdict": ("triangular_quote_response_reported"
                    if by_state["qualifying_triangle_edge"]["observations"] >= min_observations
                    else "observe_only_insufficient_aligned_triangle_edges"),
        "research_only": True,
    }
...
def main():
    ...
    print(json.dumps({
        "strategy": "crypto_triangular_arbitrage_response_replay", "input": str(args.input),
        ...
        "limitations": [
            "a triangular quote edge is not a synchronized fill, depth-aware PnL or route",
            "future BTC movement is descriptive and does not model leg completion, inventory or fees",
            "paper cost is a sensitivity hurdle, not exchange fees, latency or slippage",
        ],
