#!/usr/bin/env python3
"""Record stablecoin supply context beside a synchronized MarketBridge BTC quote."""
...
from crypto_stablecoin_liquidity_monitor import number, summarize
...
def main():
    ...
    parser.add_argument("--output", type=Path,
                        default=Path("work/crypto-stablecoin-liquidity-response.jsonl"))
    args = parser.parse_args()
    if (args.limit <= 0 or args.limit > 500 or args.growth_threshold_pct < 0
    ...
    parameters = {
        "symbols": args.symbols, "peg_type": args.peg_type, "limit": args.limit,
    ...
    with args.output.open("a", encoding="utf-8") as handle:
        for iteration in range(args.iterations):
            ...
            data = stablecoin_payload.get("data") or {}
            ...
            handle.write(json.dumps({
                ...
                "upstream_errors": ([stablecoin_payload["error"]]
                                     if stablecoin_payload.get("error") else [])
                + quote_payload.get("errors", []),
            }, ensure_ascii=False, sort_keys=True) + "\n")
            handle.flush()
            if iteration + 1 < args.iterations:
                time.sleep(args.interval_secs)
    print(json.dumps({"strategy": "crypto_stablecoin_liquidity_response_recorder",
