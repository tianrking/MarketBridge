#!/usr/bin/env python3
"""Replay OHLCV responses around point-in-time StochRSI states.

StochRSI first computes an explicit Wilder-style RSI, then measures the
current RSI inside its own trailing RSI range and optionally smooths that value
into K/D lines.  It labels extremes and K/D crosses, then measures a fixed
future response.  This sensitive oscillator is a research feature, not a
forecast, reversal or execution model.
"""
...
def rsi_series(rows, period):
    ...
    avg_gain = statistics.mean(gains[1:period + 1])
    avg_loss = statistics.mean(losses[1:period + 1])
...
def stochrsi_series(rows, rsi_period=14, stoch_period=14, smooth_k=3, smooth_d=3,
                    overbought=80.0, oversold=20.0):
    if (rsi_period <= 0 or stoch_period <= 0 or smooth_k <= 0 or smooth_d <= 0
            or not 0 <= oversold < overbought <= 100):
        return []
    series = [None] * len(rows)
    rsi = rsi_series(rows, rsi_period)
    raw_values, k_values = [None] * len(rows), [None] * len(rows)
    for index in range(len(rows)):
        if index < rsi_period + stoch_period - 1 or rsi[index] is None:
            continue
        ...
        if high <= low:
            continue
        raw = 100.0 * (rsi[index] - low) / (high - low)
        raw_values[index] = raw
        k_window = raw_values[index - smooth_k + 1:index + 1]
        k_value = _sma(k_window, smooth_k) if len(k_window) == smooth_k else None
        k_values[index] = k_value
        if k_value is None:
            continue
        d_window = k_values[index - smooth_d + 1:index + 1]
        d_value = _sma(d_window, smooth_d) if len(d_window) == smooth_d else None
        if d_value is None:
            continue
        ...
        if prior is not None:
            if prior["k"] <= prior["d"] and k_value > d_value:
                event = "bullish_kd_cross"
            elif prior["k"] >= prior["d"] and k_value < d_value:
...
def stochrsi_features(rows, index, rsi_period=14, stoch_period=14, smooth_k=3, smooth_d=3,
                      overbought=80.0, oversold=20.0):
    series = stochrsi_series(rows, rsi_period, stoch_period, smooth_k, smooth_d,
                             overbought, oversold)
    return series[index] if 0 <= index < len(series) else None


def build_observations(rows, rsi_period, stoch_period, smooth_k, smooth_d,
                       overbought, oversold, horizon_bars):
    if (rsi_period <= 0 or stoch_period <= 0 or smooth_k <= 0 or smooth_d <= 0
            or horizon_bars <= 0 or not 0 <= oversold < overbought <= 100):
        raise ValueError("invalid StochRSI periods, thresholds or horizon")
    series = stochrsi_series(rows, rsi_period, stoch_period, smooth_k, smooth_d,
                             overbought, oversold)
    observations = []
...
def summarize(observations, min_observations):
    ...
    return {
        ...
        "by_state": by_state,
        "by_event": events,
        "verdict": ("stochrsi_response_reported" if sufficient
...
def main():
    ...
    if (args.days <= 0 or not 2 <= args.limit <= 1500 or args.rsi_period <= 0
            or args.stoch_period <= 0 or args.smooth_k <= 0 or args.smooth_d <= 0
            or not 0 <= args.oversold < args.overbought <= 100
            or args.horizon_bars <= 0 or args.min_observations <= 0 or args.timeout <= 0):
        parser.error("invalid StochRSI periods, thresholds, horizon or observations")
    ...
    observations = build_observations(rows, args.rsi_period, args.stoch_period,
                                      args.smooth_k, args.smooth_d, args.overbought,
                                      args.oversold, args.horizon_bars)
    print(json.dumps({
        "strategy": "crypto_stochrsi_response_replay",
        "hypothesis": "point-in-time StochRSI extremes and K/D crosses may have different later responses from neutral controls",
        ...
        "filters": {"rsi_period": args.rsi_period, "stoch_period": args.stoch_period,
                    "smooth_k": args.smooth_k, "smooth_d": args.smooth_d,
        ...
        "limitations": [
            "RSI uses explicit Wilder smoothing and StochRSI uses a trailing RSI high/low range plus contiguous SMA K/D smoothing",
            "constant-RSI windows remain missing instead of receiving an artificial oscillator value",
            "thresholds, periods and row-count horizon are caller-supplied sensitivity parameters",
            "StochRSI is two transformations away from price and can be noisy; labels are not reversal, entry, stop or execution rules",
