#!/usr/bin/env python3
"""Replay OHLCV responses around point-in-time Vortex Indicator states.

The Vortex Indicator sums absolute cross-bar VM+/VM- movement and normalizes
each by the same trailing true-range sum.  This replay labels bullish,
bearish and balanced VI spreads, retains VI cross events and measures a fixed
future response.  It is a trend-relationship study, not a forecast or order
model.
"""

import argparse
import json
import statistics
import time
from urllib.parse import urlencode
from urllib.request import Request, urlopen


def fetch(base_url, path, params, timeout):
    query = urlencode({key: value for key, value in params.items() if value is not None})
    request = Request(f"{base_url.rstrip('/')}{path}?{query}")
    with urlopen(request, timeout=timeout) as response:
        return json.load(response)


def number(value):
    if isinstance(value, bool):
        return None
    try:
        return float(value)
    except (TypeError, ValueError):
        return None


def candle_rows(payload):
    rows = []
    for row in payload.get("candles", []):
        values = {key: number(row.get(key)) for key in ("open", "high", "low", "close")}
        timestamp = row.get("open_time_ms")
        if (isinstance(timestamp, int) and all(value is not None for value in values.values())
                and values["open"] > 0 and values["close"] > 0
                and values["high"] >= values["low"] > 0
                and values["high"] >= values["open"] and values["high"] >= values["close"]
                and values["low"] <= values["open"] and values["low"] <= values["close"]):
            rows.append({"ts_ms": timestamp, **values})
    return sorted({row["ts_ms"]: row for row in rows}.values(), key=lambda row: row["ts_ms"])


def vortex_series(rows, period=14, minimum_spread=0.05):
    if period <= 0 or minimum_spread < 0:
        return []
    series = [None] * len(rows)
    if len(rows) <= period:
        return series
    true_ranges, vm_plus, vm_minus = [None], [None], [None]
    for index in range(1, len(rows)):
        current, previous = rows[index], rows[index - 1]
        true_ranges.append(max(current["high"] - current["low"],
                               abs(current["high"] - previous["close"]),
                               abs(current["low"] - previous["close"])))
        vm_plus.append(abs(current["high"] - previous["low"]))
        vm_minus.append(abs(current["low"] - previous["high"]))
    for index in range(period, len(rows)):
        tr_window = true_ranges[index - period + 1:index + 1]
        plus_window = vm_plus[index - period + 1:index + 1]
        minus_window = vm_minus[index - period + 1:index + 1]
        if any(value is None for value in tr_window + plus_window + minus_window):
            continue
        tr_sum = sum(tr_window)
        if tr_sum <= 0:
            continue
        vi_plus = sum(plus_window) / tr_sum
        vi_minus = sum(minus_window) / tr_sum
        spread = vi_plus - vi_minus
        if spread >= minimum_spread:
            state, direction = "bullish_pressure", 1
        elif spread <= -minimum_spread:
            state, direction = "bearish_pressure", -1
        else:
            state, direction = "balanced", 0
        prior = series[index - 1]
        event = None
        if prior is not None:
            if prior["spread"] <= 0 < spread:
                event = "bullish_vi_cross"
            elif prior["spread"] >= 0 > spread:
                event = "bearish_vi_cross"
        event_direction = (1 if event == "bullish_vi_cross"
                           else -1 if event == "bearish_vi_cross" else None)
        series[index] = {
            "vi_plus": vi_plus,
            "vi_minus": vi_minus,
            "spread": spread,
            "true_range_sum": tr_sum,
            "minimum_spread": minimum_spread,
            "direction_sign": direction,
            "event_direction_sign": event_direction,
            "state": state,
            "event": event,
        }
    return series


def vortex_features(rows, index, period=14, minimum_spread=0.05):
    series = vortex_series(rows, period, minimum_spread)
    return series[index] if 0 <= index < len(series) else None


def build_observations(rows, period, minimum_spread, horizon_bars):
    if period <= 0 or minimum_spread < 0 or horizon_bars <= 0:
        raise ValueError("invalid Vortex period, spread or horizon")
    series = vortex_series(rows, period, minimum_spread)
    observations = []
    for index in range(period, len(rows) - horizon_bars):
        features = series[index]
        if features is None:
            continue
        future = rows[index + horizon_bars]
        path = rows[index + 1:index + horizon_bars + 1]
        forward = (future["close"] / rows[index]["close"] - 1.0) * 100.0
        direction = (features["event_direction_sign"]
                     if features["event_direction_sign"] is not None
                     else features["direction_sign"])
        observations.append({
            "ts_ms": rows[index]["ts_ms"],
            "future_ts_ms": future["ts_ms"],
            "state": features["state"],
            "event": features["event"],
            "features": features,
            "forward_return_pct": forward,
            "direction_aligned_return_bps": direction * forward * 100.0 if direction else None,
            "forward_absolute_return_pct": abs(forward),
            "forward_min_path_return_pct": (min(row["low"] for row in path)
                                             / rows[index]["close"] - 1.0) * 100.0,
            "forward_max_path_return_pct": (max(row["high"] for row in path)
                                             / rows[index]["close"] - 1.0) * 100.0,
        })
    return observations


def bucket_stats(rows):
    signed = [row["forward_return_pct"] for row in rows]
    aligned = [row["direction_aligned_return_bps"] for row in rows
               if row["direction_aligned_return_bps"] is not None]
    absolute = [row["forward_absolute_return_pct"] for row in rows]
    return {
        "observations": len(rows),
        "mean_forward_return_pct": statistics.mean(signed) if signed else None,
        "median_forward_return_pct": statistics.median(signed) if signed else None,
        "mean_direction_aligned_return_bps": statistics.mean(aligned) if aligned else None,
        "mean_absolute_return_pct": statistics.mean(absolute) if absolute else None,
    }


def summarize(observations, min_observations):
    states = ("bullish_pressure", "bearish_pressure", "balanced")
    by_state = {state: bucket_stats([row for row in observations if row["state"] == state])
                for state in states}
    events = {
        event: bucket_stats([row for row in observations if row["event"] == event])
        for event in ("bullish_vi_cross", "bearish_vi_cross")
    }
    pressure = by_state["bullish_pressure"]["observations"] + by_state["bearish_pressure"]["observations"]
    control = by_state["balanced"]["observations"]
    sufficient = pressure >= min_observations and control >= min_observations
    return {
        "observations": len(observations),
        "pressure_observations": pressure,
        "balanced_control_observations": control,
        "vi_cross_observations": sum(bucket["observations"] for bucket in events.values()),
        "by_state": by_state,
        "by_event": events,
        "verdict": ("vortex_response_reported" if sufficient
                     else "observe_only_insufficient_pressure_or_balanced_control"),
        "research_only": True,
    }


def main():
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--base-url", default="http://127.0.0.1:8080")
    parser.add_argument("--exchange", default="binance")
    parser.add_argument("--symbol", default="BTCUSDT")
    parser.add_argument("--market", default="perp")
    parser.add_argument("--interval", default="1h")
    parser.add_argument("--days", type=float, default=180.0)
    parser.add_argument("--limit", type=int, default=1500)
    parser.add_argument("--period", type=int, default=14)
    parser.add_argument("--minimum-spread", type=float, default=0.05)
    parser.add_argument("--horizon-bars", type=int, default=8)
    parser.add_argument("--min-observations", type=int, default=5)
    parser.add_argument("--timeout", type=float, default=30.0)
    args = parser.parse_args()
    if (args.days <= 0 or not 2 <= args.limit <= 1500 or args.period <= 0
            or args.minimum_spread < 0 or args.horizon_bars <= 0
            or args.min_observations <= 0 or args.timeout <= 0):
        parser.error("invalid Vortex period, spread, horizon or observations")
    end_ms = int(time.time() * 1000)
    start_ms = end_ms - int(args.days * 86_400_000)
    payload = fetch(args.base_url, "/v1/history/candles", {
        "exchange": args.exchange, "market": args.market, "symbol": args.symbol,
        "candle_type": "perp", "interval": args.interval, "start_ms": start_ms,
        "end_ms": end_ms, "limit": args.limit,
    }, args.timeout)
    rows = candle_rows(payload)
    observations = build_observations(rows, args.period, args.minimum_spread, args.horizon_bars)
    print(json.dumps({
        "strategy": "crypto_vortex_response_replay",
        "hypothesis": "point-in-time VI plus/minus pressure and cross events may have different later responses from balanced controls",
        "market": {"exchange": args.exchange, "market": args.market,
                   "symbol": args.symbol, "interval": args.interval, "timezone": "UTC"},
        "window": {"start_ms": start_ms, "end_ms": end_ms, "days": args.days},
        "filters": {"period": args.period, "minimum_spread": args.minimum_spread,
                    "horizon_bars": args.horizon_bars, "min_observations": args.min_observations},
        "source_counts": {"price_bars": len(rows), "observations": len(observations)},
        "observations": observations,
        "summary": summarize(observations, args.min_observations),
        "coverage": payload.get("coverage_detail"),
        "upstream_errors": [payload["error"]] if payload.get("error") else [],
        "limitations": [
            "VI uses absolute cross-bar VM plus/minus movement and a trailing true-range sum; platform initialization may differ",
            "minimum spread, period and row-count horizon are caller-supplied sensitivity parameters",
            "zero-range windows and missing bars remain unavailable rather than inferred",
            "pressure and cross labels are descriptive, not reversal, entry, stop or execution rules",
            "overlapping windows omit fees, funding, slippage, queue and fills and do not establish causality",
        ],
        "execution": "research_only_no_orders",
    }, ensure_ascii=False, sort_keys=True))


if __name__ == "__main__":
    main()
