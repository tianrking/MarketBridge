...
and near-term ATM IV may differ from the next expiry.  This monitor uses
strike-moneyness buckets as a transparent proxy because the chain summary may
not expose a comparable 25-delta for every venue.  It never prices a trade or
places an order.
"""
...
def bucket_rows(rows, underlying, atm_band, wing_min, wing_max):
    ...
    for row in rows:
        ...
        if strike is None or mark_iv is None or strike <= 0 or mark_iv <= 0:
            continue
...
def observe(base_url, currency, venue, expiry_days, atm_band, wing_min, wing_max,
    ...
    return {
        ...
        "evidence": [
            ...
            "comparable_moneyness_buckets_available" if target and target.get("put_call_skew_iv") is not None else "missing_comparable_moneyness_buckets",
            "two_expiry_term_point_available" if far else "missing_second_expiry",
        ],
        "limitations": [
            "moneyness buckets are not a universal 25-delta proxy",
