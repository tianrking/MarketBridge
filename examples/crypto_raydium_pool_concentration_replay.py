#!/usr/bin/env python3
"""Replay persistence of Raydium pool concentration/turnover states."""
...
def summarize_records(records, min_run):
    ...
    stress_states = {"high_turnover_fragmented", "high_turnover_concentrated"}
    by_pool = {}
    for (source, symbol), pools in sorted(groups.items()):
        states = [pool.get("state", "observe_only_missing_pool_metrics") for pool in pools]
        ...
        by_pool[f"{source}:{symbol}"] = {
            ...
            "longest_stress_run": longest_run(states, stress_states),
            "state_counts": {state: states.count(state) for state in sorted(set(states))},
            "verdict": (
                "persistent_raydium_pool_pressure_candidate"
                if longest_run(states, stress_states) >= min_run
                else "observe_only_no_persistent_raydium_pool_pressure"
            ),
        }
    return {
        "pools": len(by_pool),
...
def main():
    ...
    print(json.dumps({
        "strategy": "crypto_raydium_pool_concentration_replay",
        "input": str(args.input),
        "invalid_lines": invalid_lines,
        "filters": {"min_run": args.min_run},
        "summary": summarize_records(records, args.min_run),
        "limitations": [
            "Raydium API snapshots do not establish complete on-chain swap coverage",
            "page-level concentration is not a fill, LP-PnL, fee-income or slippage model",
