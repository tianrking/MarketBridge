...
class StrategyCatalogTests(unittest.TestCase):
    ...
    def test_every_categorized_python_entrypoint_is_documented(self):
        ...
        for category in CATEGORIES:
            ...
            for path in sorted((CRYPTO / category).glob("*.py")):
                if path.name in ("__init__.py", "sitecustomize.py"):
                    continue
                self.assertTrue(
    ...
    def test_examples_root_contains_documentation_only(self):
        self.assertEqual(list(EXAMPLES.glob("*.py")), [])
        for family in ("prediction", "weather"):
            self.assertTrue((EXAMPLES / family / "README.md").is_file(), family)
