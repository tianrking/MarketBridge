#!/usr/bin/env python3
"""Compare BTC responses after Orca Whirlpool risk-context states."""
...
def snapshot_state(observation):
    ...
    if "warning_or_adaptive_fee_pressure" in states:
        return "warning_or_adaptive_fee_pressure"
    if "high_turnover_whirlpool" in states:
        return "high_turnover_whirlpool"
    if "ordinary_whirlpool_state" in states:
        return "ordinary_whirlpool_state"
    if "observe_only_partial_orca_page" in states:
        return "partial_orca_page"
    return "missing_pool_data"
...
def summarize_records(records, horizon_records, min_observations):
    ...
    for index, record in enumerate(ordered):
        ...
        if current is None or future is None or current <= 0 or future <= 0:
            continue
        observations.append({"state": snapshot_state(record.get("observation") or {}),
                             "forward_return_pct": (future / current - 1.0) * 100.0})
    states = ("warning_or_adaptive_fee_pressure", "high_turnover_whirlpool",
              "ordinary_whirlpool_state", "partial_orca_page", "missing_pool_data")
    return {"by_state": {state: bucket_stats([row for row in observations if row["state"] == state])
                          for state in states},
            "aligned_forward_windows": len(observations),
            "verdict": ("orca_whirlpool_response_reported" if len(observations) >= min_observations
                        else "observe_only_insufficient_aligned_orca_snapshots"),
            "research_only": True}
...
def main():
    ...
    print(json.dumps({"strategy": "crypto_orca_whirlpool_response_replay", "input": str(args.input),
                      ...
                      "limitations": [
                          "Orca warning/adaptive-fee states are descriptive provider context, not causal evidence",
                          "BTC response does not estimate LP income, active-range slippage, fills or execution PnL",
                          "sampling gaps, tick ranges, gas, MEV, wallets and orders are not modeled",
